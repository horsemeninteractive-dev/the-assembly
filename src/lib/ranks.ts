...
const TIERS: Omit<RankTier, 'roman'>[] = [
  { name: 'Bronze',   minElo: 0,    maxElo: 999,  color: 'text-amber-700',   bg: 'bg-amber-900/20',   border: 'border-amber-800/50',  icon: '🥉' },
  { name: 'Silver',   minElo: 1000, maxElo: 1199, color: 'text-gray-300',    bg: 'bg-gray-700/20',    border: 'border-gray-500/50',   icon: '🥈' },
  { name: 'Gold',     minElo: 1200, maxElo: 1399, color: 'text-yellow-400',  bg: 'bg-yellow-900/20',  border: 'border-yellow-600/50', icon: '🥇' },
  { name: 'Platinum', minElo: 1400, maxElo: 1599, color: 'text-cyan-400',    bg: 'bg-cyan-900/20',    border: 'border-cyan-600/50',   icon: '💎' },
  { name: 'Diamond',  minElo: 1600, maxElo: 9999, color: 'text-blue-300',    bg: 'bg-blue-900/20',    border: 'border-blue-500/50',   icon: '✦'  },
];
...
export function getRankTier(elo: number): RankTier {
  const tier = TIERS.slice().reverse().find(t => elo >= t.minElo) ?? TIERS[0];
