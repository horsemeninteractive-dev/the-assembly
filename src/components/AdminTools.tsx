...
import { 
  Trash2, Megaphone, Users, Clock, Shield, AlertTriangle, 
  RefreshCw, Search, Settings, MessageSquare, Ban, UserCheck, 
  Save, Server, Activity, ChevronRight, X
} from 'lucide-react';
...
const TAB_CONFIG: { id: AdminTab; label: string; icon: any; activeBg: string; activeColor: string }[] = [
...
export const AdminTools: React.FC<AdminToolsProps> = ({ adminId, token }) => {
  ...
  const handleSearchUsers = async () => {
    ...
    try {
      ...
      const res = await fetch(apiUrl(`/api/admin/users/search?adminId=${adminId}&q=${userSearchQuery}`), {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.ok) {
        const data = await res.json();
  ...
  const handleClearRedis = () => {
    if (!confirm('Are you sure you want to purge ALL stale Redis room data? This will not affect active in-memory rooms but will prevent them from being restored on restart.')) return;
            ...
            >
              <section className="bg-yellow-500/5 border border-yellow-500/20 rounded-3xl p-6 relative overflow-hidden">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h3 className="text-xs font-mono text-primary uppercase tracking-widest font-bold">Global Broadcast</h3>
                </div>
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 px-2">
                <div className="flex items-center gap-2 flex-1">
                  ...
                  <button 
                    onClick={fetchRooms} 
                    className={cn("p-2.5 rounded-xl border border-subtle bg-white text-ghost hover:text-primary transition-all shadow-sm group", loading && "animate-spin")}
                    title="Refresh Rooms"
                  >
                  ...
                  <div key={room.id} className="bg-card/40 border border-subtle rounded-2xl p-4 flex items-center justify-between group hover:border-ghost/30 transition-all">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col">
                        <span className="text-[10px] font-mono text-faint uppercase mb-0.5">Host: {room.hostName}</span>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] text-ghost font-mono">{room.playerCount}/{room.maxPlayers} USERS</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      {confirmDelete === room.id ? (
                        ...
                        <button onClick={() => setConfirmDelete(room.id)} className="p-2.5 text-red-500/50 hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-all">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      )}
                    </div>
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                <div className="space-y-2.5">
                   <div className="text-[10px] font-mono text-faint uppercase font-bold tracking-[0.2em] mb-2 px-2">Results</div>
                  {userSearchResults.map(user => (
                    <button 
                      ...
                      className={cn(
                        ...
                        selectedUser?.id === user.id ? "border-yellow-500/50 bg-yellow-500/5 ring-1 ring-yellow-500/30" : "border-subtle"
                      )}
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <ChevronRight className={cn("w-4 h-4 transition-transform", selectedUser?.id === user.id ? "translate-x-0.5 text-yellow-500" : "text-ghost opacity-20")} />
                      </div>
                    </button>
                  ))}
                  {userSearchResults.length === 0 && !loading && (
                    <div className="py-16 text-center bg-elevated/20 border border-dashed border-subtle rounded-3xl">
                      ...
                      <p className="text-ghost text-[10px] font-mono uppercase tracking-widest italic">{hasSearched ? 'No players found matching criteria' : 'Enter criteria above'}</p>
                    </div>
                  )}
                </div>
                  ...
                  {selectedUser && (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] font-mono text-yellow-500 uppercase font-bold tracking-[0.2em]">Account Inspector</span>
                        <button onClick={() => setSelectedUser(null)} className="p-1 hover:bg-elevated rounded-lg transition-colors"><X className="w-4 h-4 text-ghost" /></button>
                      </div>

                      <div className="flex items-center gap-4 border-b border-subtle pb-6">
                        <img src={selectedUser.avatarUrl || `https://api.dicebear.com/7.x/avataaars/svg?seed=${selectedUser.username}`} className="w-16 h-16 rounded-2xl bg-elevated border-2 border-primary/20 shadow-lg" alt="" />
                        <div>
                          <div className="text-xl font-thematic text-primary uppercase tracking-wider">{selectedUser.username}</div>
                          <div className="text-[10px] font-mono text-muted-foreground break-all opacity-60">{selectedUser.id}</div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-4">
                        <div className={cn(
                          ...
                          <div>
                            <div className="text-xs font-bold text-primary flex items-center gap-2">
                              {selectedUser.isBanned ? <Ban className="w-3.5 h-3.5 text-red-500" /> : <UserCheck className="w-3.5 h-3.5 text-emerald-500" />}
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div className="bg-elevated/40 p-4 rounded-2xl border border-subtle">
                            <div className="text-[9px] font-mono text-faint uppercase mb-1.5 font-bold tracking-widest">Cabinet Points</div>
                            <input 
                              type="number" 
                              value={selectedUser.cabinetPoints} 
                              onChange={(e) => handleUpdateUser(selectedUser.id, { cabinetPoints: parseInt(e.target.value) || 0 })}
                          ...
                          <div className="bg-elevated/40 p-4 rounded-2xl border border-subtle">
                            <div className="text-[9px] font-mono text-faint uppercase mb-1.5 font-bold tracking-widest">ELO Rating</div>
                            <input 
                              type="number" 
                              value={selectedUser.stats?.elo || 1000} 
                              onChange={(e) => handleUpdateUser(selectedUser.id, { stats: { elo: parseInt(e.target.value) || 0 } })}
                              className="bg-white w-full border border-subtle rounded-xl px-3 py-2 text-xs font-mono text-black outline-none focus:ring-2 focus:ring-primary/40"
                            />
                          </div>
                        </div>
            ...
            >
              <section className="bg-card/50 border border-subtle rounded-3xl p-8 space-y-8 shadow-xl">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-xl font-thematic uppercase tracking-[0.15em] text-primary">Live Controller</h2>
                    <p className="text-[10px] text-blue-400 font-mono uppercase tracking-widest">Environment Meta-Data</p>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <div className={cn(
                    ...
                    config.maintenanceMode ? "bg-red-900/20 border-red-500/40" : "bg-elevated/30 border-subtle"
                  )}>
                    <div className="flex gap-4">
                      ...
                      <div>
                        <div className="text-sm font-bold text-primary uppercase tracking-widest">Maintenance Mode</div>
                        <div className="text-[10px] font-mono uppercase mt-0.5 opacity-60">
                          {config.maintenanceMode ? 'Blocking non-admin sessions' : 'Sessions open to all'}
                        </div>
                      </div>
                    </div>
                    <button 
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-elevated/50 p-6 rounded-3xl border border-subtle space-y-4">
                      <div className="flex justify-between items-center">
                        <div className="text-[10px] font-mono text-blue-400 uppercase font-bold tracking-widest">XP MULTIPLIER</div>
                        <span className="text-2xl font-thematic text-primary">{config.xpMultiplier}x</span>
                      </div>
                      <input 
                        type="range" min="1" max="5" step="0.5" 
                        value={config.xpMultiplier}
                        onChange={(e) => handleUpdateConfig({ xpMultiplier: parseFloat(e.target.value) })}
                        className="w-full accent-blue-500 h-2 bg-zinc-800 rounded-lg appearance-none cursor-pointer"
                      />
                    </div>
                    <div className="bg-elevated/50 p-6 rounded-3xl border border-subtle space-y-4">
                      <div className="flex justify-between items-center">
                        <div className="text-[10px] font-mono text-emerald-400 uppercase font-bold tracking-widest">IP MULTIPLIER</div>
                        <span className="text-2xl font-thematic text-primary">{config.ipMultiplier}x</span>
                      </div>
                      <input 
                        type="range" min="1" max="5" step="0.5" 
                        value={config.ipMultiplier}
                        onChange={(e) => handleUpdateConfig({ ipMultiplier: parseFloat(e.target.value) })}
            ...
            >
              <div className="bg-card/40 border border-subtle rounded-3xl flex flex-col overflow-hidden shadow-xl">
                <div className="p-4 border-b border-subtle bg-elevated/30 flex items-center justify-between">
                  <span className="text-[10px] font-mono text-faint uppercase font-bold tracking-widest">Sessions</span>
                  <button onClick={fetchRooms} className={cn("text-ghost hover:text-primary transition-colors", loading && "animate-spin")}><RefreshCw className="w-3 h-3" /></button>
                </div>
                <div className="flex-1 overflow-y-auto p-2 space-y-1 scrollbar-hide">
                  {rooms.map(room => (
                    ...
                    >
                      <div className="flex flex-col">
                        ...
                        <span className="text-[10px] font-mono opacity-50 uppercase tracking-tighter">{room.phase}</span>
                      </div>
                  ...
                  {rooms.length === 0 && <div className="p-8 text-center text-ghost text-[10px] font-mono italic">No active sessions</div>}
                </div>
              </div>
              ...
              <div className="md:col-span-2 bg-black/60 border border-subtle rounded-3xl flex flex-col overflow-hidden shadow-2xl relative">
                <div className="p-4 border-b border-subtle bg-elevated/30 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-mono text-ghost uppercase font-bold tracking-widest">
                      {selectedRoomId ? `PERSISTED_LOG: ${selectedRoomId}` : 'WAITING_FOR_SELECTION'}
                    </span>
                  </div>
                  {selectedRoomId && (
                    <button onClick={() => handleGetChatLogs(selectedRoomId)} className="text-ghost hover:text-emerald-400 transition-colors">
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-2.5 font-mono text-[11px] scrollbar-hide">
                  {chatLogs.map((log, idx) => (
                    <div key={idx} className="flex gap-4 items-start animate-in fade-in slide-in-from-left-3 duration-300">
                       <span className="text-zinc-600 shrink-0 tabular-nums">[{new Date(log.timestamp).toLocaleTimeString([], { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' })}]</span>
                  ...
                  {selectedRoomId && chatLogs.length === 0 && (
                    <div className="h-full flex flex-col items-center justify-center text-ghost/20 italic">
                      ...
                      <p className="font-mono text-[9px] uppercase tracking-widest">Terminal Idle — No Data Stream Detected</p>
                    </div>
                  )}
                  {!selectedRoomId && (
                    <div className="h-full flex flex-col items-center justify-center text-ghost/20 italic">
                      <Shield className="w-12 h-12 mb-4 opacity-10" />
                      <p className="font-mono text-[9px] uppercase tracking-widest">Admin Authorization Verified — Waiting for target</p>
                    </div>
                  )}
                </div>
