...
export const PlayerCard = React.memo(({
  p, index, gameState, isMe, isPresidentialCandidate, isPresident,
  isManyPlayers, isSpectator, isHost, stream, isVideoActive,
  speakingPlayers, playSound, setSelectedPlayerId,
}: PlayerCardProps) => {
  ...
  const handlePressStart = () => {
    ...
    pressTimerRef.current = setTimeout(() => {
      ...
      if (p.userId) { playSound('click'); setSelectedPlayerId(p.userId); }
    }, 500);
  };
    ...
    >
      ...
      {isSpectator && roleInfo && (
        <div className="absolute top-0.5 right-0.5 z-20 flex flex-col gap-0.5 items-end pointer-events-none">
          <div className={cn('px-1 py-0.5 rounded text-[8px] font-bold leading-none border border-white/20', roleInfo.bg, roleInfo.color)}>
            {roleInfo.label}
          </div>
      ...
      {p.isProvenNotOverseer && (
        <div className="absolute top-0.5 left-0.5 z-20 pointer-events-none">
          <div className="px-1 py-0.5 rounded text-[8px] font-bold leading-none border border-white/20 bg-emerald-900/80 text-emerald-300">NO</div>
        </div>
      )}
      ...
      >
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center backface-hidden">
          ...
          <div className={cn(
            ...
            {/* Desktop badges */}
            <div className={cn('flex flex-wrap gap-0.5 sm:gap-1 shrink-0', stream && isVideoActive ? 'justify-end' : 'justify-center hidden sm:flex')}>
            ...
            <div className={cn('sm:hidden flex flex-wrap gap-0.5 mt-0.5 justify-center', stream && isVideoActive && 'hidden')}>
              {!p.isAlive && <span className="px-1 py-0.5 bg-red-900/20 text-red-500 font-mono uppercase rounded text-[6px]">Dead</span>}
              {gameState.detainedPlayerId === p.id && <span className="px-1 py-0.5 bg-purple-900/20 text-purple-500 font-mono uppercase rounded text-[6px]">Detained</span>}
              {(p.isPresident || p.isPresidentialCandidate) && (
      ...
      {gameState.phase === 'Nominate_Chancellor' && isPresidentialCandidate && p.id !== socket.id && p.isAlive && (() => {
        ...
        return (
          <button
            onClick={(e) => { e.stopPropagation(); playSound('click'); socket.emit('nominateChancellor', p.id); }}
      ...
      {gameState.phase === 'Executive_Action' && isPresident && p.id !== socket.id && p.isAlive && (
        <button
          onClick={(e) => { e.stopPropagation(); playSound('click'); socket.emit('performExecutiveAction', p.id); }}
