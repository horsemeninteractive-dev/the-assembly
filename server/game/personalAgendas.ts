...
export const AGENDA_DEFINITIONS: AgendaDefinition[] = [
...

  {
    ...
    evaluate: (s, pid) => {
      const count = (s.roundHistory ?? []).filter(
        (r) => !r.chaos && r.votes.some((v) => v.playerId === pid && v.vote === 'Nay')
      ).length;
      if (count >= 3) return 'completed';
      return s.phase === 'GameOver' ? 'failed' : 'unresolved';
    },
...

  {
    ...
    evaluate: (s, pid) => {
      const count = (s.roundHistory ?? []).filter(
        (r) => r.failed && !r.chaos && r.votes.some((v) => v.playerId === pid && v.vote === 'Nay')
      ).length;
...

  {
    ...
    evaluate: (s, pid) => {
      const count = (s.roundHistory ?? []).filter((r) => !r.chaos && r.chancellorId === pid).length;
      if (count >= 2) return 'completed';
      return s.phase === 'GameOver' ? 'failed' : 'unresolved';
    },
  },

  {
    ...
    evaluate: (s, pid) => {
      const found = (s.roundHistory ?? []).some(
        (r) => r.failed && !r.chaos && (r.presidentId === pid || r.chancellorId === pid)
      );
...

  {
    ...
    evaluate: (s, pid) => {
      const served = (s.roundHistory ?? []).some(
        (r) => !r.failed && !r.chaos && r.chancellorId === pid
      );
 ...
 */
export function evaluateAllAgendas(state: GameState): Map<string, AgendaStatus> {
 ...
 */
export function getPlayerAgenda(state: GameState, playerId: string): PersonalAgenda | undefined {
