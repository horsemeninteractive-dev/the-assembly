...
export class GameEngine {
  ...
  broadcastState(roomId: string): void {
    ...
    // Strip all sensitive card fields from the base payload upfront.
    // Each will be re-added selectively per recipient below.
    ...
    for (const socketId of roomSockets) {
      ...
      // Admins and spectators get full card visibility (they already see all roles).
      ...
      const payload = {
        ...
        // Legislative hands: only the relevant player sees their actual cards.
        drawnPolicies: seeAllCards || isPresident ? drawnPolicies : [],
        chancellorPolicies: seeAllCards || isChancellor ? chancellorPolicies : [],

        // Hand history for declarations: same restriction as the live hands.
  ...
  private generateDeclaration(
    ...
    if (type === 'President') {
      ...
      while (drewSta < sta && drewCiv > 0) {
        drewCiv--;
        drewSta++;
      }
      while (drewCiv < civ && drewSta > 0) {
        drewSta--;
        drewCiv++;
      }
    }
    ...
    switch (role) {
      ...
      case 'Interdictor': {
        ...
        const target = s.players.find(
          (p) => p.id === data.targetId && p.isAlive && p.id !== president.id && p.id !== player.id
        );
        if (target) {
        ...
        const nextInterdictor = s.players.find(
          (p) => p.titleRole === 'Interdictor' && !p.titleUsed && p.isAlive && p.id !== player.id
        );
        if (nextInterdictor) {
          s.titlePrompt = {
  ...
  private async applyExecutiveAction(
    s: GameState,
    roomId: string,
    targetId: string
  ): Promise<void> {
    ...
    s.roundHistory.push({
      ...
      presDeclaration: presDecl
        ? {
            civ: presDecl.civ,
            sta: presDecl.sta,
            drewCiv: presDecl.drewCiv ?? 0,
            drewSta: presDecl.drewSta ?? 0,
          }
        : undefined,
